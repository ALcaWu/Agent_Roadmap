...
class Device(ABC):
    ...
    def power_off(self):
        ...
        if self._is_power_on:
            print(f"设备正在关机。。。")
            self._is_power_on = False
            print(f"✅ {self.model} 设备已关机")
        else:
            print(f"⚠️ {self.model} 已经是关机状态")

    def __str__(self):
        status = "开机" if self._is_powered_on else "关机"
        return f"{self.model} ({self.os_version}) - {status}"


class AndroidPhone(Device):
    def __init__(self, model, os_version, brand):
        super().__init__(model, os_version)
        self.brand = brand
        self.apps = []

    def boot(self):
        """Android启动动画"""
        print(f"   显示 {self.brand} Logo...")
        print(f"   加载 Android {self.os_version}...")

    def install_app(self, app_name):
        """安装应用"""
        if self._is_power_on:
            self.apps.append(app_name)
            print(f"已安装: {app_name}应用")
        else:
            print("请先开机")
...
class iPhone(Device):
    ...
    def boot(self):
        """iOS启动动画"""
        print(f"   显示 Apple Logo...")
        print(f"   加载 iOS {self.os_version}...")

    def sign_in_icloud(self, apple_id):
        """登录iCloud"""
        if self._is_powered_on:
            self.apple_id = apple_id
            print(f"☁️ 已登录 iCloud: {apple_id}")
        else:
            print("❌ 请先开机")
...
def main():
    ...
    # 创建设备
    pixel7 = AndroidPhone("Pixel 7", "13", "Google")
    iphone = iPhone("iPhone 14", "16")

    devices = [pixel7, iphone]
    ...
    # 2. 测试特定功能
    print("2️⃣ 测试Android特定功能:")
    pixel7.install_app("Google Maps")
    pixel7.install_app("Chrome")
    pixel7.list_apps()
    print()  # 空行
